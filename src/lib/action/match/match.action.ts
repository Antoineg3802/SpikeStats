...
.schema(
	z.object({
		teamId: z.string(),
		oponentName: z.string().min(1, "Le nom de l'équipe adverse est requis"),
		matchDate: z.string().min(1, "La date du match est requise"),
		location: z.string().optional(),
		result: z.string().optional(),
	})
)
.action(async ({ parsedInput: { teamId, oponentName, matchDate, location, result }, ctx: { user } }) => {
	if (!user) return { error: "Unauthorized" };
	...
	if (user.id !== team.ownerId && !team.teamMembers.some((member) => member.userId === user.id)) {
