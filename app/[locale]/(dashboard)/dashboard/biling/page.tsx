...
} from "@/lib/action/stripe/stripe.action";
...
export default function Page() {
	...
	const [previewSubscription, setPreviewSubscription] =
		useState<Stripe.Invoice | null>(null);
		...
		const fetchData = async () => {
			...
			if (!fullProfil) {
				...
				let parsedProfil: UserFullProfil | undefined | null =
					fullProfil?.data;
				let parsedProducts: Stripe.Product[] | undefined =
				...
				<div className="h-max w-full flex gap-3 items-stretch">
					{products.map((product, index) => {
						...
						return (
							<div
								key={index}
								className="w-1/3 p-6 border shadow-md flex flex-col gap-2 justify-between rounded-lg"
							>
								<AlertDialog>
									<h4>{product.name}</h4>
									<div>
									...
									{parsedSession.user.subscription?.active &&
									parsedSession.user.subscription
										...
										<>
											<AlertDialogContent>
												<AlertDialogHeader>
													...
													<AlertDialogDescription>
														Cette action ne pourras
														pas être annulée. Cela
														résiliera votre
														abonnement.
													</AlertDialogDescription>
												</AlertDialogHeader>
										...
										<>
											<AlertDialogContent>
												<AlertDialogHeader>
													...
													<AlertDialogDescription>
														Cette action ne pourras
														pas être annulée et cela
														changera automatiquement
														le montant de la
														prochaine échéance à{" "}
														<span className="text-primary">
															{previewSubscription
																? previewSubscription.total /
																  100
																: "0"}{" "}
															€
														</span>
														...
														{price.recurring
															?.interval ===
															"year" &&
															"Sachez qu'un abonement d'un an sera débité en une fois et implique un engagement d'un an,  vous ne pourrez pas être remboursé."}
													</AlertDialogDescription>
												</AlertDialogHeader>
												<AlertDialogFooter>
													<AlertDialogCancel>
											...
											<AlertDialogTrigger asChild>
												<Button
													variant="default"
													onClick={() =>
														handlePreviewSubscriptionClick(
															price.id
														)
													}
												>
